...
CI runs on pull requests only. The `main` ruleset requires branches to be up
to date before merge, so each squash-merged tree is the tree PR CI validated.
The release-bot version bump bypasses pull requests and is never tested on
`main`; Release owns static validation and full acceptance on latest Node 24
on the tagged commit before publication.
