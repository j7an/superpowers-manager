  ...
  try {
    ...
    if (info !== null) {
      ...
      try {
        ...
        ctx.stderr.write(
          `error: cannot remove selection state: ${statePath}\n`,
        );
        return 1;
      }
      removed = true;
    ...
    // selectionStatePath, readConfigRef, or inspect() above — re-emitting a
    // subordinate module's own diagnostic is the sanctioned form of
    // interpolation (see AGENTS.md's diagnostics convention).
    ctx.stderr.write(
