...
// Retains the first `limit` bytes and counts the rest. push() always consumes the
...
// Signal the child's whole process group. The child leads a group of its own,
// so -pid reaches descendants. Wrapped because the group is already gone in
...
export function runValidator(
  ...
  return new Promise((resolveRun) => {
    ...
    const out = new Sink(policy.maxBytesPerStream);
    const err = new Sink(policy.maxBytesPerStream);
    let settled = false;
    ...
    try {
      child = spawn(command, args, {
        ...
        // signalling the process alone leaves descendants holding inherited pipes.
        detached: true,
      });
    } catch (cause) {
      settle({
    ...
    child.on("error", (cause: NodeJS.ErrnoException) => {
      settle({ kind: "launchFailed", errno: cause.code ?? "UNKNOWN" });
    });
    child.on("close", (code) => {
...
export function configurationErrors(
  ...
  if (!VALIDATOR_COMMANDS.has(cmd) || !env.SUPERPOWERS_VALIDATOR) return [];
  return [
