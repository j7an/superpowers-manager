...
| `CLI-COMMANDS-01` | The eight named subcommands are `pin`, `track-latest`, `unpin`, `prepare`, `probe`, `install`, `update`, and `uninstall`. `pin`, `track-latest`, `unpin`, and `probe` run in-process; the other four are spawned as scripts. Except for CLI-owned arity and pin-ref checks, remaining arguments pass through unchanged to the in-process handler or the spawned script, whichever the command selects. |
