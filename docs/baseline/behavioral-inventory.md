...
| `CLI-COMMANDS-01` | The eight named subcommands are `pin`, `track-latest`, `unpin`, `prepare`, `probe`, `install`, `update`, and `uninstall`. `pin`, `track-latest`, `unpin`, `probe`, and `prepare` run in-process; the other three (`install`, `update`, `uninstall`) are spawned as scripts. Except for CLI-owned arity and pin-ref checks, remaining arguments pass through unchanged to the in-process handler or the spawned script, whichever the command selects. |
...
| `CLI-PREFLIGHT-01` | Preflight is command-specific and completes before dispatch: `pin` needs Git; `track-latest` and `unpin` have no extra tool; `prepare` needs Git, and Python only when `SUPERPOWERS_VALIDATOR` names a validator; `probe` needs Git and Codex; `install` and `update` need Git, Python, and Codex; `uninstall` needs Python and Codex. A POSIX shell is additionally required only for the three spawned commands (`install`, `update`, `uninstall`); `pin`, `track-latest`, `unpin`, `probe`, and `prepare` run in-process and need no shell. Missing requirements exit 1 without dispatch. |
| `CLI-CHILD-STATUS-01` | After successful preflight, each of the three spawned commands (`install`, `update`, `uninstall`) delegates to a script that inherits stdio and environment; its numeric exit status is the CLI exit status, and spawn failure or a signal-only result is normalized to exit 1. `pin`, `track-latest`, `unpin`, `probe`, and `prepare` run in-process instead: their handler's own returned exit code is the CLI exit status directly, with no child process, spawn, or signal involved. |
...
| `FS-GENERATED-RESOLVE-01` | Generated-plugin path validation resolves manifest-supplied paths component-wise, following symlinks and tolerating missing components, so a `..` returns to the existing prefix and resolution continues from there, while a looping component is left unresolved and resolution continues past it. Paths that resolve outside the plugin root, and paths terminating in a live symlink cycle, fail closed. Pathnames are handled without lossy decoding: an unpaired surrogate in a manifest-supplied path, a replacement character in the command-line plugin root, and an undecodable directory entry or symlink target all fail closed rather than resolving to a different file. A filesystem call that fails for any reason other than absence fails closed under its site's diagnostic, rather than being treated as a passing check. |
