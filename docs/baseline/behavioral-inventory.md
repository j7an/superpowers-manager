...
| `CLI-PREFLIGHT-01` | Preflight is command-specific and completes before dispatch: `pin` needs Git; `track-latest` and `unpin` have no extra tool; `prepare` needs Git and Python; `probe` needs Git and Codex; `install` and `update` need Git, Python, and Codex; `uninstall` needs Python and Codex. A POSIX shell is additionally required only for the four spawned commands (`prepare`, `install`, `update`, `uninstall`); `pin`, `track-latest`, `unpin`, and `probe` run in-process and need no shell. Missing requirements exit 1 without dispatch. |
| `CLI-CHILD-STATUS-01` | After successful preflight, each of the four spawned commands (`prepare`, `install`, `update`, `uninstall`) delegates to a script that inherits stdio and environment; its numeric exit status is the CLI exit status, and spawn failure or a signal-only result is normalized to exit 1. `pin`, `track-latest`, `unpin`, and `probe` run in-process instead: their handler's own returned exit code is the CLI exit status directly, with no child process, spawn, or signal involved. |
