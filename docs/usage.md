...
The built-in generated-tree validator always runs. `SUPERPOWERS_VALIDATOR_EXECUTABLE=/path/to/validator` adds an external executable after it and receives the candidate plugin root as its sole argument. It never replaces or bypasses built-in validation: either failure prevents prepared-tree replacement and harness mutation. `SUPERPOWERS_VALIDATOR` has been removed; unset it and configure an executable shebang script or executable launcher instead.
