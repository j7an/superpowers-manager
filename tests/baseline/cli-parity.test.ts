 ...
 * adapter views issue (`src/adapter.ts:795-800::const listing`, `src/adapter.ts:870::const plugins = await listingCommand`, `src/adapter.ts:882::const marketplaces = await listingCommand` — the argument
...
// fingerprint inspect calls activePluginVersionFromJson (`src/codex-json.ts:103::activePluginVersionFromJson`),
...
void test("CLI-ENV-INSTALLED-DEFAULTS-01 with no codex override and no search root the listing resolves codex from PATH and the installed fingerprint is read under $HOME/.codex", () => {
  ...
  withSandbox((sandbox) => {
    ...
    // `src/adapter.ts:839-843::cannot inspect active Codex plugin fingerprint under` rather than reporting CACHE_COMMIT.
    const upstream = createReleaseRepo(sandbox);
    const result = runCliWithoutEnvironment(
      sandbox,
      ["probe", "--porcelain"],
  ...
  // empty value survives to the manager; `src/adapter.ts:824::if (!searchRoot)` tests
  ...
  // `join(env.HOME || "/", ".codex")` (`src/adapter.ts:831::searchRoot = join`) -- the arm reached
  ...
  withSandbox((sandbox) => {
    ...
    // so the adapter's fail() text (`src/adapter.ts:839-843::cannot inspect active Codex plugin fingerprint under`) arrives verbatim.
  ...
  // one "no usable HOME" case: `src/adapter.ts:824-828::env.HOME === undefined` tests
  ...
  withSandbox((sandbox) => {
    ...
    // winning at `src/adapter.ts:823::SUPERPOWERS_INSTALLED_SEARCH_ROOT`, and the HOME branch would never be
    // reached at all.
    const result = runCliWithoutEnvironment(
      sandbox,
      ["probe", "--porcelain"],
