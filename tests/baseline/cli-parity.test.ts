 ...
 * adapter views issue (`src/adapter.ts:802-807::const listing`, `src/adapter.ts:876::const plugins = await listingCommand`, `src/adapter.ts:888::const marketplaces = await listingCommand` — the argument
...
void test("CLI-ENV-01 eleven SUPERPOWERS variables pass through", () => {
  withSandbox((sandbox) => {
    ...
    // environment before execFile (`src/adapter.ts:144::delete childEnv.NODE_OPTIONS`), landed by this
...
// fingerprint inspect calls activePluginVersionFromJson (`src/codex-json.ts:121::activePluginVersionFromJson`),
...
void test("INSTALL-ORDER-01 install prepares and validates before adapter mutation", async () => {
  {
    ...
    // no Codex command of its own (`src/adapter.ts:304-558::async function runBuild`, `runBuild`), so a
...
void test("LIFECYCLE-INTERRUPT-01 interrupted installation state fails closed", async () => {
  ...
  // (`src/adapter.ts:785-858::if (view === "fingerprint")`, the "fingerprint" view) needs a matching cached
 ...
 * `installedRootForVersion` (`src/codex-state.ts:43-50::installedRootForVersion`) builds
...
void test("CLI-ENV-CODEX-LISTING-01 the fingerprint listing uses the SUPERPOWERS_CODEX override, and resolves codex from PATH when it is unset", async () => {
  ...
  // (`src/adapter.ts:282::for (const directory of env.PATH.split(delimiter))`): an EMPTY component is KEPT, so `join("", command)`
  ...
  // `src/adapter.ts:282::for (const directory of env.PATH.split(delimiter))` is reached at all. The product CLI binds
  // `codexHarness`, whose installed-state inspection reaches the same native
  // engine (`src/codex-harness.ts:467-471::inspectInstalled: async (selection, ctx) =>`).
  //
  // Be precise about what that buys, because the next reader auditing whether
  // `src/adapter.ts:281::if (env.PATH === undefined) return false;` is reachable needs the true answer: the preflight
  ...
  // nothing shipped reaches `src/adapter.ts:281::if (env.PATH === undefined) return false;` unguarded. So this half and
  ...
  // (`src/adapter.ts:281::if (env.PATH === undefined) return false;`): an absent PATH does not synthesize a
  ...
  // `command-not-found` code the precheck raises (`src/adapter.ts:202::if (code === "ENOENT" || code === "EACCES") {`), so
  ...
  // `src/adapter.ts:281::if (env.PATH === undefined) return false;`. Deleting PATH from the CONTEXT env is not enough
...
void test("CLI-ENV-CODEX-MUTATION-01 the install mutation uses the SUPERPOWERS_CODEX override", () => {
  withSandbox((sandbox) => {
    ...
    // names the mutation path (`src/adapter.ts:578::async function runInstall`).
    assert.ok(
...
void test("CLI-ENV-INSTALLED-DEFAULTS-01 with no codex override and no search root the listing resolves codex from PATH and the installed fingerprint is read under $HOME/.codex", () => {
  ...
  withSandbox((sandbox) => {
    ...
    // `src/adapter.ts:849-853::cannot inspect active Codex plugin fingerprint under` rather than reporting CACHE_COMMIT.
    const upstream = createReleaseRepo(sandbox);
    const result = runCliWithoutEnvironment(
      sandbox,
      ["probe", "--porcelain"],
  ...
  // empty value survives to the manager; `src/adapter.ts:830::if (!searchRoot)` tests
  ...
  // `join(env.HOME || "/", ".codex")` (`src/adapter.ts:837::searchRoot = join`) -- the arm reached
  ...
  withSandbox((sandbox) => {
    ...
    // so the adapter's fail() text (`src/adapter.ts:849-853::cannot inspect active Codex plugin fingerprint under`) arrives verbatim.
  ...
  // one "no usable HOME" case: `src/adapter.ts:831-835::env.HOME === undefined` tests
  ...
  withSandbox((sandbox) => {
    ...
    // winning at `src/adapter.ts:829::SUPERPOWERS_INSTALLED_SEARCH_ROOT`, and the HOME branch would never be
    // reached at all.
    const result = runCliWithoutEnvironment(
      sandbox,
      ["probe", "--porcelain"],
...
void test("CLI-ENV-REFRESH-MODE-01 install refuses a refresh mode outside add-only and remove-add, before any Codex mutation", () => {
  ...
  // mutation. `src/adapter.ts:595::unsupported SUPERPOWERS_INSTALL_REFRESH_MODE` validates the enumeration three
  ...
  // `src/adapter.ts:640::codex marketplace add failed` and never reaches the refresh-mode branch at :672.
  ...
  // and does not need to be: `src/adapter.ts:677::if (refreshMode === "remove-add") {` issues that command
