...
const REGISTERED_SANDBOXES: WeakMap<Sandbox, string> = new WeakMap();
const SANDBOX_TOOLS = [
  "node",
  "sh",
  "git",
...
function linkHostTool(bin: string, name: string) {
  symlinkSync(hostExecutable(name), join(bin, name));
}

function assertSandboxHostTool(bin: string, name: string) {
...
function registeredRoot(sandbox: Sandbox): string {
  const root =
...
function isErrno(value: unknown, code: string) {
  return (
...
function pathEntryExists(pathValue: string) {
  try {
    lstatSync(pathValue);
    return true;
  } catch (error) {
...
function physicalPath(pathValue: string, label: string) {
...
function copyRuntimePackage(pkg: string) {
...
function writeNoopTool(sandbox: Sandbox, name: string = "codex") {
 ...
 *
 */
function writeCodexLogTool(sandbox: Sandbox): string {
  registeredRoot(sandbox);
 ...
 *
 */
function assertNoCodexContact(sandbox: Sandbox) {
  registeredRoot(sandbox);
  const tool = join(sandbox.bin, "codex");
  if (!existsSync(tool)) {
    throw new Error(
...
function removeTool(sandbox: Sandbox, name: string) {
  registeredRoot(sandbox);
  if (basename(name) !== name) throw new Error("tool name must be a basename");
  const tool = join(sandbox.bin, name);
...
function destroySandbox(sandbox: Sandbox) {
  const root = registeredRoot(sandbox);
