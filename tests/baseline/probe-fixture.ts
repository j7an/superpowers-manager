 ...
 * runCodexOperation merges process.env (`src/harnesses/codex/adapter.ts:910::const env = { ...process.env, ...context.env };`) and runGit spreads it
...
export function caseEnv(
  ...
  return {
    ...
    // (`src/harnesses/codex/adapter.ts:910::const env = { ...process.env, ...context.env };`), so this is the only channel that reaches it.
