...
} from "../../src/effective-selection.ts";

import { displaySource } from "../../src/selection.ts";
 ...
 * script's source.
 */
function fakeResolverGitDir(t: import("node:test").TestContext): {
  dir: string;
  log: string;
} {
...
function logText(log: string) {
  return existsSync(log) ? readFileSync(log, "utf8") : "";
}

function resolutionCount(log: string) {
  return logText(log)
    .split("\n")
 ...
 */
async function withResolverEnv<T>(
  gitDir: string,
  log: string,
  fn: () => Promise<T>,
): Promise<T> {
...
function assertEffective(
  selection: import("../../src/effective-selection.ts").EffectiveSelection,
  expected: {
