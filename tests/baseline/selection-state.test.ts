...
const HELPER = fileURLToPath(
  new URL("../../src/selection-state-cli.ts", import.meta.url),
);
...
function readState(state: Fixture, path?: string): HelperResult {
  return runHelper([
    "read",
    "--path",
...
function readRaw(state: Fixture, raw: string): HelperResult {
  ensureStateDirectory(state);
  writeFileSync(state.statePath, raw, "utf8");
  return readState(state);
}

function readRecord(state: Fixture, record: unknown): Record<string, string> {
  ensureStateDirectory(state);
...
function assertControlledError(result: HelperResult) {
  const lines =
...
function assertReadFails(state: Fixture, raw: string, fragment?: string) {
