...
const SIGNAL_CHILD = fileURLToPath(
  new URL("./selection-commands-signal-child.ts", import.meta.url),
);

import { runPin } from "../../src/commands/pin.ts";

import { runUnpin } from "../../src/commands/unpin.ts";

import { runTrackLatest } from "../../src/commands/track-latest.ts";

import { readSelectionState } from "../../src/selection-store.ts";

import { verifyRawCommit, readConfigRef } from "../../src/upstream.ts";
 ...
 */
function installFakeGit(dir: string, body: string): string {
 ...
 */
function assertWorkspaceParentEmpty(dir: string) {
 ...
 */
async function packagedFallback(): Promise<string> {
...
void test("pin's writer revalidates saved state after Git verification and rejects a race introduced during resolution", async () => {
  ...
  for (const conflict of ["malformed", "newer"] as const) {
...
void test("pin fails closed on malformed or newer saved state and on a credential-bearing source before any Git process runs", async () => {
  ...
  async function attemptWithExistingState(
    existingBytes: string,
  ): Promise<{ status: number; gitLog: string; statePath: string }> {
