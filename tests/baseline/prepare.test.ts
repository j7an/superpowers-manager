...
import { createCase } from "../bin/lifecycle-fixture.ts";
import {
  caseEnv,
  cloneUpstream,
  commitOf,
...
const generated = (c: { pkg: string }) => join(c.pkg, "plugins", "superpowers");

const cacheRepo = (c: CaseEnv) => join(c.dir, "cache", "superpowers");

const cacheManifest = (c: CaseEnv) =>
  join(cacheRepo(c), ".codex-plugin", "plugin.json");
 ...
 */
function assertNoLeakedInternals(stderr: string) {
 ...
 */
function seedSentinel(c: CaseEnv): string[] {
 ...
 */
function generatedManifest(c: CaseEnv): Record<string, unknown> {
  return JSON.parse(
 ...
 */
function brokenManifestUpstream(
  c: CaseEnv,
  write: (path: string) => void,
): { source: string; commit: string; manifest: string } {
 ...
 * intact.
 */
async function assertManifestRejected(
  c: CaseEnv,
  write: (path: string) => void,
  message: (manifest: string) => string,
) {
...
async function saveSelection(
  c: CaseEnv,
  record: import("../../src/selection.ts").SelectionRecord,
) {
...
// P1 — the adapter's classification wrapper (`src/harnesses/codex/adapter.ts:375::hook classification failed`). The eight inner causes
...
// with this prefix intact. Its materialization twin (`src/harnesses/codex/adapter.ts:381::hook materialization failed`) is
...
void test("prepare runs the executable validator inside the staging workspace", async () => {
  ...
  const workspace = printed as string;
 ...
 * argument.
 */
function unpoisonWorkspaces(parent: string) {
 ...
 */
function assertOrder(haystack: string, needles: string[]) {
  let previous = -1;
...
void test("a post-success workspace cleanup failure keeps the prepared outcome and every line before it", async () => {
  ...
  try {
    ...
    //    always emits this on the stdout channel, `src/harnesses/codex/adapter.ts:486::generated plugin validation passed` --
...
void test("prepare keeps hostile git output off its stream on both fetch branches", async () => {
   ...
   */
  const nonRepository = (c: CaseEnv): string => {
...
// asserting nothing. `tests/container.sh:7-11::actual_uid=` refuses a root container, but
