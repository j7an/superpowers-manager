...
// P1 — the adapter's classification wrapper (`src/harnesses/codex/adapter.ts:416::hook classification failed`). Ported from
...
// with this prefix intact. Its materialization twin (`src/harnesses/codex/adapter.ts:424::hook materialization failed`) is
...
void test("a post-success workspace cleanup failure keeps the prepared outcome and every line before it", async () => {
  ...
  try {
    ...
    //    always emits this on the stdout channel, `src/harnesses/codex/adapter.ts:572::generated plugin validation passed` --
