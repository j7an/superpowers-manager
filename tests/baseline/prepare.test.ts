...
// P1 — the adapter's classification wrapper (`src/adapter.ts:394::hook classification failed`). Ported from
...
// with this prefix intact. Its materialization twin (`src/adapter.ts:403::hook materialization failed`) is
...
void test("a post-success workspace cleanup failure keeps the prepared outcome and every line before it", async () => {
  ...
  try {
    ...
    //    always emits this on the stdout channel, `src/adapter.ts:554::generated plugin validation passed` --
