 ...
 * then one `codex plugin marketplace list --json` (`src/harnesses/codex/adapter.ts:679::async function runOwnership(`).
 ...
 * `runUninstall` (`src/harnesses/codex/adapter.ts:611::const { pluginPresent, marketplacePresent } = input;`). That leaves one
 ...
 * (`src/harnesses/codex/adapter.ts:626::plugin not installed; skipping`, :665).
 *
...
void describe("uninstall commands", { concurrency: true }, () => {
  ...
  void test("legacy-only state is never mutated and leaves guidance (:234-242)", async () => {
    ...
    // of each (`src/harnesses/codex/adapter.ts:626::plugin not installed; skipping`, :665) — so together they pin both the call
  ...
  void test("both present: both removed, plugin before marketplace (:261-289)", async () => {
    ...
    // `plugin list --json` (`src/harnesses/codex/adapter.ts:679::async function runOwnership(`) and one
  ...
  void test("marketplace remove fails after the plugin remove succeeds (:428-455)", async () => {
    ...
    // adapter issues only when both flags are true (`src/harnesses/codex/adapter.ts:611::const { pluginPresent, marketplacePresent } = input;`).
    assert.ok(has(codex, "plugin remove superpowers@superpowers-manager"));
    assert.ok(has(codex, "plugin marketplace remove superpowers-manager"));
