...
// These cases execute native runUninstall directly with an injected recording
// adapter for the seam-dependent cases below, matching the native unit suite.
// src/cli.ts is the maintained CLI entry; see tests/bin/command-context.ts for
// the in-process adapter seam.
...
// `git` is deliberately absent; `node` is appended separately below.
const NO_GIT_TOOLS = [
  "awk",
  "cat",
  "cut",
 ...
 */
function hasLine(text: string, needle: string): boolean {
  return text.split("\n").includes(needle);
}
 ...
 */
function assertNoRemoves(log: string[]) {
  assert.ok(
    log.length > 0,
 ...
 * then one `codex plugin marketplace list --json` (`src/harnesses/codex/adapter.ts:664::async function runOwnership(`).
 ...
 */
function ownershipInspections(codex: string[]): number {
 ...
 * before issuing any Codex command — `requireCodex` failing inside
 * `runUninstall` (`src/harnesses/codex/adapter.ts:566::const { pluginPresent, marketplacePresent } = input;`). That leaves one
 ...
 */
function assertNoAdapterUninstall(codex: string[], message: string) {
  assert.ok(
    codex.length > 0,
 ...
 * (`src/harnesses/codex/adapter.ts:581::plugin not installed; skipping`,
 * `src/harnesses/codex/adapter.ts:598::marketplace not registered; skipping`).
 *
 ...
 */
function assertTmpEmpty(caseEnv: import("./lifecycle-fixture.ts").CaseEnv) {
...
void describe("uninstall commands", { concurrency: true }, () => {
  void test("source guards: no Codex ownership leaks into shared code (:9-16)", () => {
    ...
    const lifecycle = readFileSync(
      join(ROOT, "src", "harnesses", "codex", "lifecycle.ts"),
      "utf8",
    );
    assert.ok(
  ...
  void test("selection-independent recovery: malformed selection, no git, unsupported update control (:162-190)", async () => {
    // The double permits only ownership and removal operations. Malformed
    // saved selection and a Git-free PATH prove uninstall needs neither
    // selection resolution nor update-control inspection.
      ...
      if (joined === "inspect --view ownership") {
        ...
        return successResult(
          ...
          codexOwnershipInspection(
            present ? "manager" : "neither",
            { pluginPresent: present, marketplacePresent: present },
            [],
          ),
          [],
        );
  ...
  // This PATH contains only dirname. Besides proving Python is unnecessary,
  // it checks that runScript launches Node via absolute process.execPath.
  // The exact Codex sequence below proves the command reached its subject.
  void test("no python3 on PATH: uninstall runs anyway and reaches Codex (:192-212 retired)", async () => {
    ...
    // The exact Codex sequence makes this absence assertion non-vacuous.
    assert.ok(
      !out.includes("required command not found: python3"),
      `uninstall must no longer require python3:\n${out}`,
    );
    // Require the full ownership, removal, and verification sequence;
    // an empty log cannot satisfy it.
  ...
  void test("missing Codex: controlled ownership-inspect failure (:214-232)", async () => {
    // Calls `runUninstall` in-process, with the
  ...
  void test("legacy-only state is never mutated and leaves guidance (:234-242)", async () => {
    ...
    // of each (`src/harnesses/codex/adapter.ts:581::plugin not installed; skipping`,
    // `src/harnesses/codex/adapter.ts:598::marketplace not registered; skipping`) — so together they pin both the call
    ...
    // `git show ad56569a4c161e7b122967442e2b026eeb6395f6:scripts/core/lifecycle.sh:52::'Run: npx superpowers-wrapper@0.1.1 uninstall' \`,77, naming the last superpowers-wrapper
  ...
  void test("both present: both removed, plugin before marketplace (:261-289)", async () => {
    // Re-anchored onto codex.log, keeping
    // `runScript` -- unlike the two cases above, every live claim here has a
    // Codex-level footprint. `inspect --view ownership` issues one
    // `plugin list --json` (`src/harnesses/codex/adapter.ts:664::async function runOwnership(`) and one
  ...
  void test("remove is a no-op: verify-after detects the still-present target (:394-410)", async () => {
    // Suppress both fake removals' state changes while acknowledging the calls.
  ...
  void test("marketplace remove fails after the plugin remove succeeds (:428-455)", async () => {
    ...
    // adapter issues only when both flags are true (`src/harnesses/codex/adapter.ts:566::const { pluginPresent, marketplacePresent } = input;`).
    assert.ok(has(codex, "plugin remove superpowers@superpowers-manager"));
    assert.ok(has(codex, "plugin marketplace remove superpowers-manager"));
