...
import { schemaFor, validateConfig } from "./lifecycle-config.ts";
...
export function loadFixtureConfig(
  ...
  try {
    ...
    process.stderr.write(`fixture: ${(error as Error).message}\n`);
    process.exitCode = 93;
    return { ...defaults, __failed: true };
  }
 ...
 *
 *   inspect --view fingerprint -> plugin list --json          (`src/harnesses/codex/adapter.ts:802-805::const listing`)
 *   inspect --view ownership   -> plugin list --json,         (`src/harnesses/codex/adapter.ts:876-879::const plugins`)
 ...
 *
 */
export function injectSpuriousMutation(
  ctx: FakeContext,
 ...
 *
 */
export function tripwireTriggered(
  ctx: FakeContext,
