...
// nobody listed anywhere. Source ownership separates production from tests,
// and an independent bounded walk detects ignored manager-generated JavaScript.
 ...
 */
function capture(command: string, args: string[], what: string): string {
  try {
 ...
 */
function pathListOf(name: string): string[] {
 ...
 */
function resolvedProjectFiles(project: string, projectDir: string): string[] {
  const raw = capture(
    join(TOOL_BIN, "tsc"),
...
void test(
  "every maintained test TypeScript file is typechecked by the tests project",
  { skip: HOST_ONLY },
  () => {
    const expected = universe().filter(
      (path) => path.startsWith("tests/") && path.endsWith(".ts"),
    );
    assert.deepEqual(
...
void test(
  "every maintained production TypeScript file is typechecked by the production project",
  { skip: HOST_ONLY },
  () => {
    const expected = universe().filter(
...
void test(
  "the checked projects cover the maintained TypeScript universe without JavaScript",
  { skip: HOST_ONLY },
  () => {
    const files = universe();
    assert.deepEqual(
      files.filter((path) => JAVASCRIPT.test(path)),
      [],
    );
    const checked = [
      ...resolvedProjectFiles("tsconfig.json", "."),
      ...resolvedProjectFiles("tests/tsconfig.json", "tests"),
    ].sort();
...
// Git's tracked/untracked universe excludes ignored output. Inspect manager
// ownership separately without following symlinks into other trees. Dependencies,
// Git metadata, unrelated worktrees and generated upstream content are excluded.
function managerJavaScript(root: string): string[] {
  const found: string[] = [];
  function visit(directory: string): void {
    for (const entry of readdirSync(join(root, directory), {
      ...
      if (
        ["node_modules", ".git", ".worktrees"].includes(entry.name) ||
        path === "plugins/superpowers" ||
        path.startsWith("plugins/.superpowers.prepare.")
      )
        continue;
      if (entry.isDirectory()) visit(path);
      else if (JAVASCRIPT.test(path)) found.push(path);
    }
  }
  visit("");
  return found.sort();
...
void test("manager-owned checkout paths contain no JavaScript, including ignored output", () => {
  assert.deepEqual(managerJavaScript(ROOT), []);
});

void test("the ownership check catches ignored generated JavaScript in an external checkout", () => {
  const root = mkdtempSync(join(tmpdir(), "spw-owned-sources-"));
  try {
    execFileSync("git", ["init", "--quiet", root]);
    writeFileSync(join(root, ".gitignore"), "dist/\n");
    mkdirSync(join(root, "dist"));
    writeFileSync(
      join(root, "dist", "stale.js"),
      "// ignored generated output\n",
    );
    assert.equal(
      execFileSync("git", ["check-ignore", "dist/stale.js"], {
        cwd: root,
        encoding: "utf8",
      }),
      "dist/stale.js\n",
    );
    assert.deepEqual(managerJavaScript(root), ["dist/stale.js"]);
  } finally {
