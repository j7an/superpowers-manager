 ...
 */
function isRegularFile(relPath: string) {
  try {
...
const textContentCases: Array<[string, string, boolean]> = [
  ...
  ["AGENTS.md", "`src/`", true],
  ["AGENTS.md", "`dist/`", true],
  ["CONTRIBUTING.md", "sh tests/container.sh", true],
  ["CONTRIBUTING.md", "Layers 1-3 stay offline and hermetic", true],
  ["CONTRIBUTING.md", "Layer 4 is the Docker acceptance path", true],
  [
    "CONTRIBUTING.md",
    "The package scripts above are alternative iteration selectors",
    true,
  ],
  ["CONTRIBUTING.md", "pnpm install --frozen-lockfile", true],
  ["CONTRIBUTING.md", "node src/cli.ts", true],
  ["CONTRIBUTING.md", "toolchain", true],
  ["README.md", "Omitting `--harness` selects Codex.", true],
  ["docs/usage.md", "superpowers-manager pin v6.1.1", true],
  ["docs/usage.md", "superpowers-manager track-latest", true],
  ["docs/usage.md", "superpowers-manager unpin", true],
  ["docs/usage.md", "selection commands save intent only", true],
  ["docs/usage.md", "`SUPERPOWERS_REF` is an invocation-only override", true],
  [
    "docs/usage.md",
  ...
  ["RELEASING.md", "node tests/tools/pack.ts --out-dir", true],
...
void test("bootstrap: text-content assertions", () => {
  for (const path of ["AGENTS.md", "RELEASING.md"]) {
    const content = read(path);
    assert.match(content, /SPW_NATIVE_NODE_VERSION=24\.12\.0/);
    assert.match(content, /SPW_NATIVE_NODE_VERSION=24(?:\s|\x60)/);
    assert.match(content, /SPW_PACKAGE_NODE/);
    assert.match(content, /24\.0\.0/);
  }
...
function extractSection(
  ...
  const bodyStart = (match.index as number) + match[0].length;
