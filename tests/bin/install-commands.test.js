...
void describe("install commands", { concurrency: true }, () => {
  ...
  // Arming it here and still getting a normal fresh install proves the
  // in-process port never reaches tests/bin/install-fakes.js's adapter role
  // at all, tripwire armed or not — install-fakes.js's own runAdapter now
  // refuses that role unconditionally (tests/bin/lifecycle-fakes.js's
  ...
  // would carry "fixture: install must not spawn the adapter" instead.
