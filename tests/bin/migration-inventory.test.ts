...
const DECLARED: Record<string, string[]> = {
  "adapter-protocol.md": [
    "tests/baseline/cli-parity.test.ts",
  ...
  "prepare.md": [
    ...
    "tests/unit/commands-prepare.test.ts",
    "tests/unit/atomic.test.ts",
  ],
  "probe.md": [
    "tests/baseline/probe.test.ts",
    "tests/unit/commands-probe.test.ts",
    "tests/unit/status.test.ts",
  ],
  "ref-resolution.md": ["tests/baseline/ref-resolution.test.ts"],
  "selection-commands.md": ["tests/baseline/selection-commands.test.ts"],
  "selection-state.md": ["tests/baseline/selection-location.test.ts"],
  "uninstall-commands.md": ["tests/bin/uninstall-commands.test.ts"],
  "workflows.md": [
...
const FROZEN_SHELL_RESOLUTION: Record<string, string> = {
  "install-commands.md":
 ...
 */
function frozenRegionLines(source: string, name: string): string[] {
...
function frozenDigest(lines: string[]): string {
...
function digestRegistry(name: string): Record<string, unknown> {
  let raw;
  try {
...
const STATED_COUNT_MARKER_CHECK_EXEMPT: Record<
  "merges" | "retirements",
  ReadonlySet<string>
> = {
...
const STATED_COUNT_REQUIRED: Record<
  "merges" | "retirements",
  ReadonlySet<string>
> = {
 ...
 * are correct.
 */
function assertNetArithmeticSelfConsistent(source: string, name: string) {
...
function parseStatedCount(token: string): number {
...
function assertStatedCountMatchesMarkers(
  ...
  markerComparisonExempt: ReadonlySet<string>,
) {
 ...
 * (`tests/migration-inventory/workflows.md:500::Node-absence: renamed the top-level`, `tests/migration-inventory/workflows.md:506::single leading space`, `tests/migration-inventory/workflows.md:519::forbidden publish configuration at`) that heading inference reads as entries 1-3. That
 ...
 * matched pair.
 */
function mappedLinesOf(
  outer: { start: number; end: number; lines: string[] },
  inner: { start: number; end: number; lines: string[] } | null,
  name: string,
): string[] {
 ...
 */
function entryNumbers(lines: string[], label: string): number[] {
  const found = [];
...
function assertExactlyOneThrough(
  numbers: number[],
  expected: number,
  label: string,
) {
...
for (const name of Object.keys(DECLARED)) {
  void test(`migration inventory reconciles: ${name}`, () => {
    ...
    const declared: {
      shellOriginal: unknown;
      portOnly: unknown;
      ports: unknown;
    } = parsed as any;
    assert.ok(
      Number.isInteger(declared.shellOriginal) &&
        (declared.shellOriginal as number) > 0,
      `${name}: shellOriginal must be a positive integer`,
    );
    assert.ok(
      Number.isInteger(declared.portOnly) && (declared.portOnly as number) >= 0,
    ...
    const portEntries = Object.entries(
      declared.ports as Record<string, unknown>,
    );
    assert.ok(
    ...
    const shellOriginal = declared.shellOriginal as number;
    const portOnlyCount = declared.portOnly as number;
    ...
    for (const [portPath, expectedSites] of portEntries) {
      assert.ok(
        Number.isInteger(expectedSites) && (expectedSites as number) > 0,
