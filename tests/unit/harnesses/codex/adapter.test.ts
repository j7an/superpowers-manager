 ...
 */
async function buildWorkspace(t: import("node:test").TestContext) {
  ...
  // `build` generates both from `--fallback-manifest` (`src/harnesses/codex/adapter.ts:395::manifestSource === "upstream" ? upstreamManifest : fallbackManifest,`,
  // `src/harnesses/codex/adapter.ts:485::cannot copy fallback manifest template into candidate`), so anything written here is overwritten before validation runs.
  await writeFile(
    ...
    async () => {
      ...
      try {
        const operations = [
          ...
          ["inspect", "inspect-failed", () => codexInspectOwnership(context)],
        ] as const;
...
// call (`src/harnesses/codex/adapter.ts:432::const rawManifestBytes = await readFile(candidateManifest);`) must surface exactly `cannot read manifest JSON
...
void test("a manifest overlay read fails closed when the file changes between the two reads", () => {
  ...
  assert.throws(
    ...
    (error) => {
      const typed = error as TypeError & { code?: unknown };
...
// The install reconciliation read (`src/harnesses/codex/adapter.ts:623::registeredRoot = marketplaceRootFromJson(`) is the destructive
...
void test("mapCodexLaunchFailure carries a validated errno, guards free-form codes, and still peels off ENOENT/EACCES", () => {
  ...
  // `message`), verified by reading src/harnesses/codex/adapter.ts's `fail` and
...
void test("runCommand strips NODE_OPTIONS and NODE_PATH from the child env", async (t) => {
  ...
  const script = join(scratch, "print-env.ts");
  await writeFile(
    script,
...
// FOUR independent booleans, not two. `src/harnesses/codex/adapter.ts:776::const managerPresent = pluginPresent || marketplacePresent;` computes
...
// response schema's tolerance. src/harnesses/codex/adapter.ts emits `missing` unconditionally
...
/**
 * Drive the adapter install operation to `src/harnesses/codex/adapter.ts:673::marketplace ${MARKETPLACE_NAME} was removed but re-adding failed.`, the one
 ...
 */
async function reAddFailureRun(t: import("node:test").TestContext) {
  ...
  const result = await codexInstall(packageRoot, {
    ...
    env: {
      ...
      // executor's shell: `src/harnesses/codex/adapter.ts:592::const refreshMode = codexInstallRefreshMode(env);` enumerates only "add-only"
      ...
      // exercises is the marketplace branch at `src/harnesses/codex/adapter.ts:647::} else if (!(await pathsEqual(packageRoot, registeredRoot))) {`, which is
      // gated on pathsEqual alone and reads no refresh mode. "add-only" is
      // the default (`src/harnesses/codex/adapter.ts:592::const refreshMode = codexInstallRefreshMode(env);`) and so the value these witnesses
...
void test("ADAPTER-CONTROLLED-FAILURE-01 a controlled failure carries its error and its hints in order, yields no result, and returns status 1", async (t) => {
  ...
  // witness that. `src/harnesses/codex/adapter.ts:673::marketplace ${MARKETPLACE_NAME} was removed but re-adding failed.` is the one in-process failure with
