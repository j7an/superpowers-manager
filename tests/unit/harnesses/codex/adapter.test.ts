...
async function buildWorkspace(t: import("node:test").TestContext) {
  ...
  // `build` generates both from `--fallback-manifest` (`src/harnesses/codex/adapter.ts:354::manifestSource === "upstream" ? upstreamManifest : fallbackManifest,`,
  // `src/harnesses/codex/adapter.ts:441::cannot copy fallback manifest template into candidate`), so anything written here is overwritten before validation runs.
  await writeFile(
...
// call (`src/harnesses/codex/adapter.ts:388::const rawManifestBytes = await readFile(candidateManifest);`) must surface exactly `cannot read manifest JSON
...
// The install reconciliation read (`src/harnesses/codex/adapter.ts:531::registeredRoot = marketplaceRootFromJson(`) is the destructive
...
// FOUR independent booleans, not two. `src/harnesses/codex/adapter.ts:664::const managerPresent = pluginPresent || marketplacePresent;` computes
...
/**
 * Drive the adapter install operation to `src/harnesses/codex/adapter.ts:578::marketplace ${MARKETPLACE_NAME} was removed but re-adding failed.`, the one
...
async function reAddFailureRun(t: import("node:test").TestContext) {
  ...
  const result = await codexInstall(packageRoot, {
    ...
    env: {
      ...
      // executor's shell: `src/harnesses/codex/adapter.ts:505::const refreshMode = codexInstallRefreshMode(env);` enumerates only "add-only"
      ...
      // exercises is the marketplace branch at `src/harnesses/codex/adapter.ts:552::} else if (!(await pathsEqual(packageRoot, registeredRoot))) {`, which is
      // gated on pathsEqual alone and reads no refresh mode. "add-only" is
      // the default (`src/harnesses/codex/adapter.ts:505::const refreshMode = codexInstallRefreshMode(env);`) and so the value these witnesses
...
void test("ADAPTER-CONTROLLED-FAILURE-01 a controlled failure carries its error and its hints in order, yields no result, and returns status 1", async (t) => {
  ...
  // witness that. `src/harnesses/codex/adapter.ts:578::marketplace ${MARKETPLACE_NAME} was removed but re-adding failed.` is the one in-process failure with
