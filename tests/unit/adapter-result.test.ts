...
void test("ADAPTER-TERMINAL-01 a C0, DEL, or C1 control in any terminal-facing failure string is refused", async (t) => {
  ...
  // any byte but NUL and slash, and `src/harnesses/codex/adapter.ts:809::if (commandFailed(listing)) {` interpolates that
  ...
  await t.test("refused at the CLI without a traceback", () => {
    ...
    try {
      ...
      // (`src/harnesses/codex/adapter.ts:264::async function listingCommand(`). That record is what the hoist withholds, so
