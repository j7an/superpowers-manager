...
void test("writeAdapterFailure throws on an unsafe string and never emits the value", () => {
  ...
  const cases: readonly [
    string,
    string,
    import("../../src/adapter-result.ts").AdapterOutcome,
  ][] = [
    [
      "message",
...
// (`git show 41c99390f51a0cbeb552ab0a0bff26fc1c5c07df:tests/test_adapter_protocol.py:473-528::test_rejects_terminal_controls_in_terminal_facing_protocol_strings` at fd94d7d).
...
void test("ADAPTER-TERMINAL-01 a C0, DEL, or C1 control in any terminal-facing failure string is refused", async (t) => {
  ...
  await t.test("byte ingress via appendBytes", () => {
    ...
    for (const [label, bytes, escaped] of [
      ...
      const log = new AdapterMessageLog();
      log.appendBytes("stderr", Uint8Array.from(bytes as number[]));
      assert.deepStrictEqual(
        log.snapshot(),
        [{ channel: "stderr", text: escaped }],
  ...
  // any byte but NUL and slash, and `src/harnesses/codex/adapter.ts:808-812::commandFailed(listing)` interpolates that
  ...
  await t.test("refused at the CLI without a traceback", () => {
    ...
    try {
      ...
      // (`src/harnesses/codex/adapter.ts:251-260::async function listingCommand`). That record is what the hoist withholds, so
      ...
      const run = spawnSync(
        process.execPath,
        [fileURLToPath(new URL("../../src/cli.ts", import.meta.url)), "probe"],
        {
          encoding: "utf8",
...
void test("ADAPTER-SURROGATE-01 a surrogate code point in any terminal-facing failure string is refused without leaking a traceback", async (t) => {
  await t.test("refused at writeAdapterFailure", () => {
    ...
    // and the verification hint alike (`git show 41c99390f51a0cbeb552ab0a0bff26fc1c5c07df:tests/test_adapter_protocol.py:544::udc9b`
    // at fd94d7d).
