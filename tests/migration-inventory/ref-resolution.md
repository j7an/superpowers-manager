...
Source read in full (242 lines). Current native port:
`tests/baseline/ref-resolution.test.ts`.
...
(`tests/baseline/traceability.test.ts:15`). It exercises
...
1. The builder's `REPO=` output names a directory that is a git repository
   (`:21`). Port: `tests/baseline/ref-resolution.test.ts:224`.
...
5. With `SUPERPOWERS_REF` unset, `spw_config_ref` returns the packaged
   ...
   Port: `tests/baseline/ref-resolution.test.ts:239-244` (`readConfigRef`
   direct call).
...
6. `spw_manifest_version_for_ref latest-release latest-release v6.0.3
   ...
   `tests/unit/upstream.test.ts:159-171`'s "manifestVersionForRef reproduces
   ...
   live in tests/unit/upstream.test.ts").
7. `spw_manifest_version_for_ref main ref main
   ...
   6: `tests/unit/upstream.test.ts:181-189` exercises this exact tuple against
...
8. `resolveRef` selects the greatest stable tag (`v1.2.3`) for
   ...
   `tests/baseline/ref-resolution.test.ts:250-255`.
...
13. `resolveRef` falls through to the generic ref lookup for a branch name
    ...
    `tests/baseline/ref-resolution.test.ts:287-288`.
...
15. The persistent cache actually holds the requested commit object after a
    ...
    `tests/baseline/ref-resolution.test.ts:312-317` (see "Counting rules
...
26. The interrupted fetch must actually reach the signal fixture before the
    ...
    `tests/baseline/ref-resolution.test.ts:436-442` (`waitForMarker` plus the
...
27. The interrupted fetch's own proof workspace (`superpowers-manager.fetch.*`)
    ...
    `tests/baseline/ref-resolution.test.ts:478-483`.
...
29. The interrupted invocation is specifically the resolver's own inner
    ...
    `tests/baseline/ref-resolution.test.ts:461-476`.
...
32. A source with no stable tags unexpectedly succeeding at `latest-release`
    ...
    `assert.rejects` at `tests/baseline/ref-resolution.test.ts:502-507`, same
...
```json inventory
{
  "shellOriginal": 39,
  "portOnly": 0,
  "ports": { "tests/baseline/ref-resolution.test.ts": 7 }
}
```
...
- Port (`tests/baseline/ref-resolution.test.ts`): 7 static `test(` call
...
- Reconciliation: 31 of 39 shell items are mapped; 4 are retired and 4 record
  ...
  `tests/unit/upstream.test.ts`'s existing `manifestVersionForRef` table) —
...
Current ports: `tests/baseline/ref-resolution.test.ts` (7 static `test(` call sites).
