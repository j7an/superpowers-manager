...
2. **-8, double-counted.** `assert_effective`'s eight-line *definition*
   ...
   *call sites* (`:141,153,162,171,180,190,199,209,218,231,241,252,263,277`),
   ...
   nine `SPW_*` environment exports are all set under `set -u`. It has no
   ...
   `assert_effective` call that invokes it (the call is the item), and the
   port covers the property it protects structurally: `EffectiveSelection`
   (`src/effective-selection.ts`) declares all eight fields `readonly` and
   non-optional, so "every field is populated" is a compile-time guarantee
   rather than a runtime check to assert.
...
11. Every base absent unexpectedly succeeding is itself the failure
    ...
    subshell). **Merged**, same rationale as item 7. Port: `:341-344`.
...
13. `spw_usage_error 'bad arguments'` unexpectedly succeeding is itself the
    ...
    2)` at `tests/baseline/selection-location.test.js:356`, which is strictly
...
14. `spw_usage_error` exits `2` (`:69`). Port: `:356`.
15. `spw_usage_error`'s stderr is exactly `error: bad arguments` (`:70`).
    Port: `:357-358` (the port asserts the full stdout/stderr shape in one
...
16. Absent state resolves the packaged default ref, tag kind, one resolver
    ...
    package-default ...`). Port: `tests/baseline/selection-location.test.js:369-383`.
17. `SPW_SAVED_MODE` is `none` for absent state (`:143`). Port: `:384`.
...
22. Track-latest state resolves `latest-release` to a distinct tag and commit
    ...
    describes. Port: `tests/baseline/selection-location.test.js:449-461`.
23. `SPW_SAVED_MODE` is `track-latest` (`:182`). Port: `:462`.
24. `SUPERPOWERS_REF` overrides track-latest's ref while the source stays
    saved (`:184-191`). Port: `:466-481`.
25. `SUPERPOWERS_UPSTREAM_URL` overrides track-latest's source while the ref
    ...
    (`:193-200`). Port: `:485-500`.
26. Both override together (`:202-210`). Port: `:504-520`.

### Complete ref precedence — pinned state (`:212-254`)

27. Pinned state reuses its verified identity without querying the resolver
    (`:213-219`). Port: `tests/baseline/selection-location.test.js:526-540`.
...
30. `SPW_SAVED_RESOLVED_REF` equals the saved pin's resolved ref (`:222`).
    Port: `:543`.
31. `SPW_SAVED_COMMIT` equals the saved pin's commit (`:223`). Port: `:544`.
32. `SUPERPOWERS_REF` overrides a pin's ref, falling through to resolution
    (`:225-232`). Port: `:548-563`.
...
34. `SUPERPOWERS_UPSTREAM_URL` overrides a pin's source while its ref/commit
    stay saved (`:235-242`). Port: `:568-583`.
35. The resolver log is empty when only the pin's source is overridden
    (`:243`). Port: `:584`.
36. Both override together over a pin (`:245-252`). Port: `:588-604`.
...
37. An environment ref containing a shell glob character resolves as data,
    ...
    `tests/baseline/selection-location.test.js:618-633`.
38. A raw-commit saved pin derives `raw-commit` resolution kind without
    resolver access (`:266-278`). Port: `:638-661`.
...
40. Malformed saved state (`schema_version: 2`) unexpectedly succeeding is
    ...
    `assert.rejects` at `tests/baseline/selection-location.test.js:677-689`,
    same rationale as item 7.
41. The malformed-state diagnostic includes `schema_version must equal
    integer 1` (`:296`). Port: `:677-689`.
...
45. The resolver log stays empty when source validation fails first (`:310`).
    Port: `:710`.
46. Displaying a credential-bearing source redacts it (`:311`). Port: `:711`.
47. Displaying the official source shows it verbatim (`:312`). Port: `:712`.
48. `spw_selection_state` ignores ambient `NODE_OPTIONS` (`:314-322`). No
    ...
    Port: `tests/baseline/selection-location.test.js:726-750`.
49. A missing `dist/selection-state-cli.js` unexpectedly succeeding is itself
    ...
    its own assertion rather than merged. Port: `:758-776` (`:776`
    specifically).
50. The missing-helper stderr is exactly one line (`:332`). Port: `:778`.
...
- Port (`tests/baseline/selection-location.test.js`): 5 static `test(` call
  ...
  mapped (seven recorded merges, at items 7, 9, 11, 13, 40, and 43, each a
