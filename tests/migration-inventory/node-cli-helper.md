...
```json inventory
{
  "shellOriginal": 10,
  "portOnly": 0,
  "ports": { "tests/unit/harnesses/codex/adapter.test.ts": 19 }
}
```
...
- Port (`tests/unit/harnesses/codex/adapter.test.ts`): 19 static `node:test` cases are present
...
Current ports: `tests/unit/harnesses/codex/adapter.test.ts` (19 static `test(` call sites).
