...
```json inventory
{
  "shellOriginal": 10,
  "portOnly": 0,
  "ports": { "tests/unit/adapter.test.ts": 21 }
}
```
...
- Port (`tests/unit/adapter.test.ts`): 21 static `node:test` cases are present
...
Current ports: `tests/unit/adapter.test.ts` (21 static `test(` call sites).
