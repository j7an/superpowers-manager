...
Source read in full (47 lines). Current native port: `tests/unit/harnesses/codex/lifecycle.test.ts`.
...
1. `[ -z "$output" ]` for `neither` (`:12-13`). Port:
   `tests/unit/harnesses/codex/lifecycle.test.ts:29-33`.
...
11. `[ -z "$output" ]` for `neither` (`:30-31`). Port:
    `tests/unit/harnesses/codex/lifecycle.test.ts:45-49`.
...
13. `grep -Fxq 'Legacy superpowers-wrapper Codex state remains installed.'`
    for `legacy` (`:36-37`). Port: `tests/unit/harnesses/codex/lifecycle.test.ts:51-59`.
14. `grep -Fxq 'Run: npx superpowers-wrapper@0.1.1 uninstall'` for `legacy`
    (`:38-39`). Port: `tests/unit/harnesses/codex/lifecycle.test.ts:51-59`.
15. `grep -Fxq 'Legacy superpowers-wrapper Codex state remains installed.'`
    for `both` (`:36-37`). Port: `tests/unit/harnesses/codex/lifecycle.test.ts:51-59`.
...
1. **New.** `requireNoLegacyState` returns `{ kind: "unknown", message:
   ...
   Port: `tests/unit/harnesses/codex/lifecycle.test.ts:66-75`. No shell counterpart — the
...
2. **New.** `reportLegacyState` returns `{ kind: "unknown", message:
   "unknown adapter identity state: garbage" }` for an unrecognised state.
   Port: `tests/unit/harnesses/codex/lifecycle.test.ts:66-75`. No shell counterpart, same
   rationale as item 1.
3. **New.** `requireNoLegacyState` returns `{ kind: "unknown", message:
   "unknown adapter identity state: " }` for an empty identity state. Port:
   `tests/unit/harnesses/codex/lifecycle.test.ts:77-86`. No shell counterpart, same
   rationale as item 1.
...
5. **New.** `requireManagedUpdateControl("managed")` returns `{ ok: true }`.
   Port: `tests/unit/harnesses/codex/lifecycle.test.ts:124-126`. No counterpart in
...
6. **New.** `requireManagedUpdateControl("unsupported")` returns `{ ok: false,
   ...
   `tests/unit/harnesses/codex/lifecycle.test.ts:128-133`. No counterpart in either driver,
...
7. **New.** `requireManagedUpdateControl` rejects an unrecognised capability
   ...
   `tests/unit/harnesses/codex/lifecycle.test.ts:135-140`. No counterpart in either driver,
   same rationale as item 5.
8. `verifyInstalledFingerprint` returns `ok: true` for an exact 40-character
   commit match. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:142-156`. No counterpart
...
9. `verifyInstalledFingerprint`'s `stdout` is exactly `["desired_commit=…",
   ...
   Port: `tests/unit/harnesses/codex/lifecycle.test.ts:142-156`. No counterpart in
...
10. `verifyInstalledFingerprint`'s `stderr` is `[]` for the same exact-match
    case. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:142-156`. No counterpart in
...
11. `verifyInstalledFingerprint` returns `ok: true` for the seven-character
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:158-168`. No counterpart in
...
12. `verifyInstalledFingerprint` returns `ok: false` with `stderr` naming
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:170-177`. No counterpart in
...
13. Same case's `stdout` is `[]`. Port:
    `tests/unit/harnesses/codex/lifecycle.test.ts:170-177`. No counterpart in either driver,
...
14. Same case's `stderr` is exactly `["error: installed manager fingerprint
    ...
    *call* failing (`"call-failed"`, this item's own branch, unchanged) from
    ...
    items 26-28 below (`tests/unit/harnesses/codex/lifecycle.test.ts:272-287`, "an unparseable
...
15. `verifyInstalledFingerprint` returns `ok: false` on a stale/mismatched
    commit. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:179-191`. No counterpart in
    `tests/test_codex_state_units.sh`. Counterpart in
...
16. Same case's `stderr` is exactly `["error: installed manager fingerprint
    ...
    reinstalling"]`. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:179-191`. No
...
17. `verifyInstalledFingerprint` returns `ok: false` when the fingerprint is
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:193-207`. No counterpart in
    `tests/test_codex_state_units.sh`. Counterpart in
...
18. Same case's `stderr` is exactly `["error: installed manager fingerprint
    ...
    Port: `tests/unit/harnesses/codex/lifecycle.test.ts:193-207`. No counterpart in
    `tests/test_codex_state_units.sh`. Partial counterpart in
...
19. `verifyInstalledFingerprint` returns `ok: false` with no hint line when
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:209-217`. No counterpart in either driver:
...
20. Same case's `stderr` has exactly one line (no `hint:` line appended).
    Port: `tests/unit/harnesses/codex/lifecycle.test.ts:209-217`. No counterpart in either
    driver, same rationale as item 19.
21. `verifyUninstalledResources` returns `{ ok: true }` when both resources
    are absent. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:219-226`. No counterpart
    in `tests/test_codex_state_units.sh`. Counterpart in
...
22. `verifyUninstalledResources` rejects a surviving plugin with `"owned
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:228-238`. No counterpart in
    `tests/test_codex_state_units.sh`. Counterpart in
...
23. `verifyUninstalledResources` rejects a surviving marketplace with `"owned
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:240-250`. No counterpart in
    `tests/test_codex_state_units.sh`. Counterpart in
...
24. `verifyUninstalledResources` fails closed with `"expected a Boolean
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:252-265`. No counterpart in
    `tests/test_codex_state_units.sh`. No counterpart in
...
25. `verifyUninstalledResources` fails closed when the inspection itself
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:267-270`. No counterpart in
    `tests/test_codex_state_units.sh`. No counterpart in
...
27. Same case's `stderr` is exactly `["error: cannot parse installed manager
    ...
    not "inspection failed". Port: `tests/unit/harnesses/codex/lifecycle.test.ts:272-287`.
...
28. Same case's `stdout` is exactly `[]`. Port:
    `tests/unit/harnesses/codex/lifecycle.test.ts:272-287`. No counterpart in
    `tests/test_codex_state_units.sh`. Partial counterpart in
...
29. **New, this commit.** `verifyInstalledFingerprint` fails closed on a
    ...
    Port: `tests/unit/harnesses/codex/lifecycle.test.ts:289-301` ("a non-string fingerprint
...
30. Same case's `stderr` is exactly `["error: cannot parse installed manager
    ...
    Port: `tests/unit/harnesses/codex/lifecycle.test.ts:289-301`. No shell counterpart is
...
31. **New, this commit.** `verifyUninstalledResources` returns `ok: false`
    ...
    unpinned. Port: `tests/unit/harnesses/codex/lifecycle.test.ts:303-312` ("an unreadable
...
33. **New, this commit.** `verifyUninstalledResources` returns `ok: false`
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:314-325` ("the marketplace Boolean check
...
34. Same case's failure `message` is exactly `"expected a Boolean adapter
    ...
    `plugin`). Port: `tests/unit/harnesses/codex/lifecycle.test.ts:314-325`. No counterpart
...
35. **This commit; removes a port-only divergence rather than adding one.**
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:327-338` ("a non-object resources falls
...
36. Same case's failure `message` is exactly `"expected a Boolean adapter
    ...
    `tests/unit/harnesses/codex/lifecycle.test.ts:327-338`. Before spec §6.2.3 item 3a, the
...
- Port (`tests/unit/harnesses/codex/lifecycle.test.ts`): 26 static `test(` call sites,
  ...
  object rather than writing text line by line). `portOnly: 36` is the frozen
  historical count of rows 1-36 above; it is not a count of current active
  lifecycle cases or assertions. Items 1-4 cover the `*)` arm neither shell
  case statement ever reached, and items 5-25 record the former port-only
  lifecycle coverage. Items 26-36 record the later reconciliation evidence.
  Item 25 now maps its controlled failed ownership outcome to `normalizers
  preserve controlled native failure outcomes unchanged` in
  `tests/unit/harnesses/codex/harness.test.ts`; the helper-only generic
  reading-diagnostic records 31-32 are retired. The surviving fingerprint,
  ownership, Boolean-key, and malformed-resources assertions remain in the
  current 26-case lifecycle suite. Slice 4c's `marketplace-reconcile.md`
  continues to map `tests/test_marketplace_reconcile.sh` assertions onto the
  corresponding retained behavior.
  ...
  ***Amended 2026-08-12:*** *these five cases were previously carried as five
  ...
  existing `tests/unit/harnesses/codex/lifecycle.test.ts` range, exactly as the 15/16, 17/18
