...
2. Exit status is `1` when `dist/cli.js` is absent (`:46-50`). Port:
   `tests/bin/bin-dispatch.test.js:30`.
...
9. `pin v6.1.1` → logs `pin v6.1.1 ref=` (`:80-82`). **Retired** (PR 11.5,
    ...
    covered by `tests/bin/bin-dispatch.test.js:101` and recorded as port-only
    ...
    `tests/bin/dispatch-fixture.js`'s `pinUpstreamEnv`.
10. `track-latest` → logs `track-latest  ref=` (`:84-86`). **Retired** (PR
    ...
    in-process property ("succeeds without ever reaching its script") is
    covered by `tests/bin/bin-dispatch.test.js:85` and recorded as port-only
...
11. `unpin` → logs `unpin  ref=` (`:88-90`). **Retired** (PR 11.5): `unpin`
    ...
    `tests/bin/bin-dispatch.test.js:93` (moved from `:81` when Task 6 added
...
12. `install --dry-run` → logs `install --dry-run ref=` (`:92-94`). Port:
    `:81`, `ROUTING_CASES[2]`. **Index updated** (PR 11.5, Task 7) from
...
15. Running `bogus` must not succeed (`:107-109`, `if run_bin bogus; then
    ...
    `assert.equal(result.status, 2)` at `:117`, which is strictly stronger
...
16. Exit status is `2` for the unknown subcommand `bogus` (`:110-111`). Port:
    `:117`.
17. Stderr contains `unknown subcommand: bogus` (`:112`). Port: `:118`.
18. Stderr contains `usage:` (`:113`). Port: `:119`.
19. The dispatch log is empty — nothing was dispatched (`:114`). Port: `:120`.

### A stray flag must not fall through to update (`:116-122`)

20. Exit status is `2` for the stray flag `--porcelain` (`:118-119`). Port:
    `:127`.
21. Stderr contains `unknown subcommand: --porcelain` (`:120`). Port: `:128`.
22. Stderr contains `usage:` (`:121`). Port: `:129`.
23. The dispatch log is empty (`:122`, bare `[ ! -s "$log" ]`). Port: `:130`.

### `--help` and `--version` (`:124-136`)

24. Exit status is `0` for `--help` (`:125-126`). Port: `:137`.
25. Stdout contains `usage:` for `--help` (`:127`). Port: `:138`.
26. Stderr is empty for `--help` (`:128`, bare `[ ! -s "$tmpdir/help-err" ]`).
    Port: `:139`.
27. `--version` prints exactly `9.9.9-test` (`:129-130`). Port: `:145`.
28. `--version` through a symlinked bin (as npm/npx invoke bins) prints
    exactly `9.9.9-test` (`:133-136`). Port: `:155`.
...
30. `SUPERPOWERS_REF=abc123` reaches the dispatched script: log line
    `update  ref=abc123` (`:153`). Port: `:182-185` (**merged** — see item
    31).
31. `SUPERPOWERS_VALIDATOR=/tmp/custom-validator.py` reaches the dispatched
    ...
    Port: `:182-185`. Items 30-31 are two separate `grep -Fqx` calls in the
...
40. `uninstall` dispatches with `git` absent from `PATH` (loop iteration 3).
    Port: `:246`, `NO_GIT_CASES[0]`. **Index updated** (PR 11.5, Task 6) from
...
47. The dispatch log is empty — missing codex must not dispatch `install`
    (`:205`). Port: `:366`. The shell proves this with a `probe` override
    ...
    (`dispatch-fixture.js`'s `loggingStub`, `:112-126`) *also* logs
    ...
    `:366` would catch it. It is sound; it is just less visually obvious
...
48. `pin v6.1.1` dispatches with `codex` absent from `PATH` (`:208-209`,
    ...
    `tests/bin/bin-dispatch.test.js:403` and recorded as port-only item 38.
...
51. `prepare` dispatches with `codex` absent from `PATH` (shell loop
    iteration 3). Port: `:383`, `NO_CODEX_CASES[0]`. **Index updated**
...
52. Exit status is `1` when `scripts/uninstall` is missing (`:220`, bare
    `[ "$rc" -eq 1 ]`). Port: `:421`.
53. Stderr contains `missing script` (`:221`). Port: `:422`.
...
1. `ROUTING_CASES.length === 5` (`:71-75`), asserted once at module load,
...
4. Routing case `pin v6.1.1`: `result.status === 0`. **Relocated** (PR 11.5,
   ...
   own standalone case, `tests/bin/bin-dispatch.test.js:107`. Same underlying
...
5. Routing case `track-latest`: `result.status === 0`. **Relocated** (PR
   ...
   `tests/bin/bin-dispatch.test.js:87`. Same underlying property
...
6. Routing case `unpin`: `result.status === 0`. **Relocated** (PR 11.5):
   ...
   standalone case, `tests/bin/bin-dispatch.test.js:95` (moved from `:83`
...
7. Routing case `install --dry-run`: `result.status === 0` (`:80`,
...
13. `NO_GIT_CASES` iteration `track-latest`: `result.status === 0`.
    **Relocated** (PR 11.5, Task 6): `track-latest` left `NO_GIT_CASES` (see
    ...
    `tests/bin/bin-dispatch.test.js:261`. Same underlying property; same
...
14. `unpin` succeeds with `git` absent: `result.status === 0`.
    **Relocated** (PR 11.5): `unpin` left `NO_GIT_CASES` (see item 39's
    retirement note) for its own standalone case,
    `tests/bin/bin-dispatch.test.js:270` (moved from `:228`-equivalent
...
15. `NO_GIT_CASES` iteration `uninstall`: `result.status === 0` (`:251`).
...
16. `unpin` succeeds in-process with `python3` absent: `result.status === 0`
    (`:288`). **Relocated** (PR 11.5) from a bare `run_bin unpin >/dev/null`
...
17. `NO_CODEX_CASES` iteration `pin v6.1.1`: `result.status === 0`.
    **Relocated** (PR 11.5, Task 7): `pin` left `NO_CODEX_CASES` (see item
    ...
    `tests/bin/bin-dispatch.test.js:409`. Port-only — the shell's standalone
...
18. `NO_CODEX_CASES` iteration `track-latest`: `result.status === 0`.
    **Relocated** (PR 11.5, Task 6): `track-latest` left `NO_CODEX_CASES`
    ...
    `tests/bin/bin-dispatch.test.js:393`. Same underlying property; same
    rationale as before, just no longer a loop iteration.
19. `unpin` succeeds with `codex` absent: `result.status === 0`.
    **Relocated** (PR 11.5): `unpin` left `NO_CODEX_CASES` (see item 50's
    retirement note) for its own standalone case,
    `tests/bin/bin-dispatch.test.js:399` (moved from `:305` when Task 6
    ...
    iteration.
20. `NO_CODEX_CASES` iteration `prepare`: `result.status === 0` (`:383`,
...
21. **New** (PR 11.5). Routing case `unpin`: `result.log` is empty
    (`tests/bin/bin-dispatch.test.js:98`, moved from `:86` when Task 6 added
...
22. **New** (PR 11.5). `unpin` with `git` absent: `result.log` is empty
    (`:271`, moved from `:223`-equivalent position). Port-only, same
    rationale as item 21.
23. **New** (PR 11.5). `unpin` with `python3` absent: `result.log` is empty
    (`:289`). Port-only, same rationale as item 21.
24. **New** (PR 11.5). `unpin` with `codex` absent: `result.log` is empty
    (`:400`, moved from `:306`-equivalent position). Port-only, same
    rationale as item 21.
25. **New** (PR 11.5). `unpin` succeeds with no POSIX shell on `PATH`:
    `result.status === 0` (`:298`). Port-only, with no shell counterpart of
...
26. **New** (PR 11.5). `unpin` succeeds with no POSIX shell on `PATH`:
    `result.log` is empty (`:299`). Port-only, same rationale as items 21
    and 25.
27. **New** (PR 11.5, Task 6). Routing case `track-latest`: `result.log` is
    empty (`tests/bin/bin-dispatch.test.js:90`). Port-only, with no shell
...
28. **New** (PR 11.5, Task 6). `track-latest` with `git` absent: `result.log`
    is empty (`:262`). Port-only, same rationale as item 27.
29. **New** (PR 11.5, Task 6). `track-latest` with `codex` absent:
    `result.log` is empty (`:394`). Port-only, same rationale as item 27.
30. **New** (PR 11.5, Task 6). `track-latest` succeeds with `python3` and no
    POSIX shell on `PATH`: `result.status === 0` (`:316`). Port-only, with
...
31. **New** (PR 11.5, Task 6). `track-latest` succeeds with `python3` and no
    POSIX shell on `PATH`: `result.log` is empty (`:317`). Port-only, same
    rationale as item 30.
32. **New** (PR 11.5, Task 7). `pin` fails preflight when `git` is absent
    from `PATH`: `result.status === 1` (`tests/bin/bin-dispatch.test.js:212`).
...
33. **New** (PR 11.5, Task 7). Same case: stderr contains
    `required command not found: git` (`:213`). Port-only, same rationale as
    item 32.
34. **New** (PR 11.5, Task 7). Same case: `result.log` is empty (`:214`).
    Port-only, same rationale as item 32.
35. **New** (PR 11.5, Task 7). `pin` succeeds in-process with `python3`
    absent from `PATH`: `result.status === 0`
    (`tests/bin/bin-dispatch.test.js:337`). Port-only, with no shell
    ...
    — matching `track-latest`'s items 30-31 rather than `unpin`'s items
    16/23. Needs real git resolution to succeed (see
    `tests/bin/dispatch-fixture.js`'s `pinUpstreamEnv`), unlike every other
    case recorded in this port-only section.
36. **New** (PR 11.5, Task 7). Same case: `result.log` is empty (`:338`).
    Port-only, same rationale as item 35.
37. **New** (PR 11.5, Task 7). Routing case `pin`: `result.log` is empty
    (`tests/bin/bin-dispatch.test.js:110`). Port-only, with no shell
    counterpart of any kind, same rationale as item 21: the shell always
    ...
    it.
38. **New** (PR 11.5, Task 7). `pin` with `codex` absent: `result.log` is
    empty (`:410`). Port-only, same rationale as item 37.
...
- Port (`tests/bin/bin-dispatch.test.js`): 30 static `test(` call sites (3 of
  ...
  table they used to occupy — expanding to 34 runtime cases), carrying 46 of
  the 53 shell assertions 1:1-mapped (two recorded merges: item 15 into the
  port's `status === 2` check, and items 30-31 into the port's two-line
  `assert.deepEqual`), plus **8 retired items** (9, 10,
  11, 38, 39, 41, 49, 50 — items 9, 10, and 38, 49 each asserted that `pin`
  or `track-latest` "dispatches ... and logs `<name>  ref=`"; items 11, 39,
  and 50 each asserted the analogous condition for `unpin`; item 41 asserted
  ...
  tested), plus 38 port-only assertions (37 additive `result.status === 0` /
  `result.log` checks the shell left implicit under `set -e` or that have no
  shell counterpart at all, plus one structural array-length guard) with no
  shell counterpart.
- Reconciliation: 1:1 for 46 of the 53 shell items, two merges (noted above),
  8 retirements (noted above) — not drops, since a retirement is recorded
  with its own note explaining why no replacement assertion is possible,
  rather than silently disappearing. The 38 port-only assertions are
